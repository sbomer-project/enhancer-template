...
LOCAL_CHART_PATH="helm/{{ project_slug }}-chart"
...
echo "--- Building local {{ project_slug }} image inside of Minikube ---"
bash ./hack/build-local-{{ project_slug }}-into-minikube.sh
...
target_name = '{{ project_slug }}-chart'
...
helm upgrade --install sbomer-release "./$PLATFORM_DIR" \
    ...
    --set {{ project_slug }}-chart.image.repository=localhost/{{ project_slug }} \
    --set {{ project_slug }}-chart.image.tag=latest \
    --set {{ project_slug }}-chart.image.pullPolicy=Never \
    --set {{ project_slug }}-chart.config.kafka.bootstrapServers=sbomer-release-kafka:9092 \
    --set {{ project_slug }}-chart.config.kafka.schemaRegistryUrl=http://sbomer-release-apicurio:8080/apis/registry/v2 \
    --set {{ project_slug }}-chart.config.storage.internalUrl=http://sbomer-release-manifest-storage-service-chart:8080 \
    --set {{ project_slug }}-chart.config.otel.endpoint=http://sbomer-release-otel-lgtm:4317 \
    --set {{ project_slug }}-chart.config.otel.protocol=grpc

echo "--- Forcing Rolling Restart to pick up new local image ---"
kubectl rollout restart deployment -n $NAMESPACE -l app.kubernetes.io/name={{ project_slug }}-chart || true
