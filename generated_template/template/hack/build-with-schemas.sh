...
if [ "$OS" = "Linux" ]; then
    ...
    SOCKET_PATH=$(podman info --format '{% raw %}{{.Host.RemoteSocket.Path}}{% endraw %}')
    ...
    status=$(podman machine info --format "{% raw %}{{.Host.MachineState}}{% endraw %}" 2>/dev/null)
    ...
    export DOCKER_HOST="unix://$(podman machine inspect --format '{% raw %}{{.ConnectionInfo.PodmanSocket.Path}}{% endraw %}')"
...
if [ ! -d "$SCHEMA_REPO_DIR/.git" ]; then
  ...
  echo "Schema repo git tracking not found. Ensuring clean setup..."

  # If the folder exists but isn't a valid Git repo (like the Copier sandbox copy),
  # wipe it out first so git clone can execute cleanly without path conflicts.
  if [ -d "$SCHEMA_REPO_DIR" ]; then
    rm -rf "$SCHEMA_REPO_DIR"
  fi
